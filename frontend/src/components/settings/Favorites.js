...
export default function Favorites ({ setSelectedSetting}) {
    ...
    //From const user we get all the favorites that is listed to that user
    ...
    const setSelectedHelper = (selectedFunction , values , value , row ) => {
        ...
        //Here we have full list of variants
        ...
        if (value.includes("#")) {
            //Here we are changing the colour
           ...
           let newColors = []
            variants.map(variant => {
                ...
                && variant.size === value 
                && variants[selectedVariants].style === variant.style) {
    ...
    const handleDelete = row => {
        // here row is id .. i.e row is the number
        setLoading(row)
        axios.delete(process.env.GATSBY_STRAPI_URL + `/favorites/${row}` ,{
            ...
            setLoading(null)
            console.error(error)
            dispatchFeedback(setSnackbar({status : "error" , message : "There was problem removing the selected product from favorites."}))
        })
    }
    

    //Set up columns for the dataGrid table
    const columns = [{ 
                        field : "item" , 
                        headerName : "Item" , 
                        width : 250 , 
                     ...
                     return (
                        <Grid container direction='column'>
                            <Sizes sizes={sizes} selectedSize={selectedSizes[row.id]} setSelectedSize={(size) =>
                                 setSelectedHelper(setSelectedSizes , selectedSizes , size , row.id)}/>
                            <Swatches colors={colors} selectedColor={selectedColors[row.id]} setSelectedColor = {(color) => 
                                setSelectedHelper(setSelectedColors , selectedColors , color , row.id )}/>
                        </Grid>
                     )}} ,
                     { 
                        field : "quantity" ,
    ...
    //Empty array dependancy , it only runs when component first mounts
    //This axios request is done to the backend route i.e in the favorite controller and data is sent back 
    useEffect(() => {
        axios.get(process.env.GATSBY_STRAPI_URL + "/favorites/userFavorites" , {
            ...
            response.data.forEach(favorite => {
                ...
                //For each of the favorites we will initialize selectedvariant , selectedSize , selectedColors based on the variant that
                //is actually favorited
            ...
            console.error(error)
            dispatchFeedback(setSnackbar({ status : "error" , message : "There was problem getting the favorite products"}))
        })
    },[])
