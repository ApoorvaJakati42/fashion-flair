...
export default function ListOfProducts({products , content , layout , page , productsPerPage , filterOptions}) {
    ...
    const FrameHelper = ({Frame , product , variant}) => {
        ...
        const {loading , error , data} = useQuery(GET_DETAILS , {
            variables : { id : product.node.strapiId } //id is passes from line 24 as argument
        })
    ...
    return (
        <Grid item container direction={matchesSM ? "column" : "row"} alignItems={matchesSM ? "center" : "undefined"} classes={{root : classes.productContainer}}>
            { 
                content.slice((page - 1) * productsPerPage , page * productsPerPage).map(item => (
                    <FrameHelper Frame={layout === "grid" ? ProductFrameGrid : ProductFrameList} 
                    key={item.variant.id} 
